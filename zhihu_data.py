...
zhihu_collections_base_urls = [
'http://www.zhihu.com/collection/34131770',
'http://www.zhihu.com/collection/36881062',
'http://www.zhihu.com/collection/36842610',
'http://www.zhihu.com/collection/20025828',
...
def save_origin_url(fname, origin_url):
	f = open('file_url.data','a')
	f.write('%s\t%s\n' % (fname.encode('utf-8','ignore'),origin_url))
	f.close()
	
def save_answer(title, author, content,origin_url, dir='data'):
	...
	save_origin_url(fname, origin_url)
	# process image
...
def get_visited_urls():
	return open('links.data','r').read().split('\n')
...
def get_collection(base_url):
	...
	sleep_time = 5
	sub_path = path.join('data',path.basename(base_url))
	...
	for zhihu_collections_url in get_collections_urls_from_base_url(base_url):
		...
		for zm_item in zm_items:
			...
			if content_dom is None:
				...
				save_answer(title, author, header + content,href, sub_path)
